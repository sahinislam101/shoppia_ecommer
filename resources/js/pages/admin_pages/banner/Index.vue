  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    <div class="mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div class="relative w-full md:w-1/3">
        <input v-model="search" type="text" placeholder="Search banner..."
      ...
      <table class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
        ...
        <tbody>
          <tr v-for="banner in props.banners.data" :key="banner.id"
            ...
            <td class="px-6 py-4">
              <img :src="banner.image" alt="Banner Image" class="w-16 h-16 object-cover rounded shadow" />
            </td>
            ...
            <td class="px-6 py-4 space-x-2">
              <Link :href="`/banners/${banner.slug}/edit`"
                class="font-medium text-blue-600 dark:text-blue-400 hover:underline">Edit</Link>
              <Link :href="`/banners/${banner.slug}`" method="delete" as="button"
                class="font-medium text-red-600 dark:text-red-400 hover:underline">Delete</Link>
            </td>
          </tr>
          <tr v-if="props.banners.data.length === 0">
            <td colspan="7" class="text-center py-6 text-gray-500 dark:text-gray-400">No banners found.</td>
          </tr>
        </tbody>
      </table>
    </div>

    <div v-if="props.banners.last_page > 1" class="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6">
      <div class="text-sm text-gray-500 dark:text-gray-400">
        Showing {{ props.banners.from }} to {{ props.banners.to }} of {{ props.banners.total }} results
      </div>
      <div class="flex flex-wrap gap-2">
        <button v-if="props.banners.current_page > 1" @click="handlePaginate(props.banners.current_page - 1)"
        ...
        <button v-for="page in props.banners.last_page" :key="page" @click="handlePaginate(page)"
          :class="[ 
        ...
        <button v-if="props.banners.current_page < props.banners.last_page"
          @click="handlePaginate(props.banners.current_page + 1)"
          class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-blue-100 hover:text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition">
          Next
        </button>
      </div>
